...
The server generates an OpenAPI 3.x spec via the `openapi` Cargo feature (utoipa 5). The spec at `apps/server/openapi.json` is **committed to the repo** and must be regenerated and committed whenever the API changes. (`apps/docs/public/openapi.json` is generated by the docs CI at build time and is not committed.)
