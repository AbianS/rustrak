...
-- build has no dispatcher for them, so they are deleted before copying.
-- Child rows would cascade at delete time: alert_rule_channels via
-- ON DELETE CASCADE, alert_history via ON DELETE SET NULL.
