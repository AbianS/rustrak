...
DingTalk and WeCom bots configured with a **custom keyword** (自定义关键词) or
an IP allowlist reject messages that do not contain the keyword — put it in
the template's static text. DingTalk bots also require the `access_token` (and
optionally `timestamp`/`sign`) as URL query parameters; put the full URL from
the bot settings in **URL**.
