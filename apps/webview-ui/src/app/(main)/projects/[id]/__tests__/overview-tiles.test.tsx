...
async function renderCrashFree() {
  const { CrashFreeTile } = await import('../overview-tiles');
  render(await CrashFreeTile({ projectId: 1, period: '24h' }));
}

async function renderSessionHealth() {
  const { SessionHealthTile } = await import('../overview-tiles');
  render(await SessionHealthTile({ projectId: 1, period: '24h' }));
}
