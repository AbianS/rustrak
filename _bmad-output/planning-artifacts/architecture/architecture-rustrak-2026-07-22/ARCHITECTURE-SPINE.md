...
| Naming - files | kebab-case. Biome configures `useFilenamingConvention` at error level, but **no script in the repo invokes Biome**: the root `lint` task fans out to per-package `lint` scripts and only `apps/server` defines one. Wiring `lint` and `format:check` for `apps/webview-ui` is part of AD-9's work, without which this convention is unenforced in CI. Next.js special filenames are the standing exception. |
