...
**Execution:**
- [x] `apps/docs/scripts/generate-versions.mjs` -- new script: read `content/changelog/*.mdx` with `gray-matter`, sort newest first (date desc, slug desc), write `public/versions.json` as `{ latest, versions: [{ version, title, description, date, url }] }`. Strip the leading `v` from `version` so it matches `/health/version` output. `url` is `https://rustrak.github.io/rustrak/changelog#<slug>`. -- the feed the notice consumes
...
- [x] `apps/webview-ui/src/actions/version-check.ts` -- new Server Action `getUpdateInfo()`: bail early when disabled; resolve current version from `getServerVersion()` falling back to `APP_VERSION`; fetch the feed; filter entries newer than current; return `{ current, latest, description, url } | null`. Wrap everything in try/catch returning null. -- follows the existing `getServerVersion()` pattern in `actions/server.ts`
- [x] `apps/webview-ui/src/components/update-banner.tsx` -- new Client Component: a `fixed` pill that expands on hover or focus. Reads and writes `localStorage['rustrak:update-dismissed']`, renders nothing until mounted, hides when the stored value equals `latest`. -- dismissal keyed to the version so a newer release re-surfaces it
- [x] `apps/webview-ui/src/app/(main)/layout.tsx` -- await `getUpdateInfo()` and render `<UpdateBanner />` when non-null -- one notice for the whole authenticated app
...
**Acceptance Criteria:**
- Given a docs build, when it completes, then `apps/docs/out/versions.json` exists, parses, and its `versions[0].version` matches the newest changelog entry with no `v` prefix.
