 ...
 * frame and the next.
 */
export function close(id: number, result: unknown) {
...
/** A `<DialogProvider>` mounting or unmounting, so `createDialog`'s `open()`
 *  can warn in development when none is mounted. */
